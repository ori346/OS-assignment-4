...
struct inode * dereference_link_ip(struct inode *);
...
sys_chdir(void)
{
  ...
  begin_op();
  if(argstr(0, path, MAXPATH) < 0 || (ip = dereference_link(path)) == 0){
    end_op();
    return -1;
  }
  ilock(ip);
...
struct inode * dereference_link(char * path){
  struct inode * output = namei(path);
  if(output == 0)
    return 0; 
  ilock(output);
...
struct inode * dereference_link_ip(struct inode * output){ 
  int deref = MAX_DEREFERENCE;
  char buf[MAXPATH]; 
  int bufsize = 0;
